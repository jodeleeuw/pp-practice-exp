...
jsPsych.plugins["dialog"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    if (Array.isArray(trial.button_html)) {
      if (trial.button_html.length == trial.choices.length) {
        ...
        console.error('Error in dialog plugin. The length of the button_html array does not equal the length of the choices array');
      }
    } else {
    ...
    html += '<div id="jspsych-dialog-btngroup">';
    for (var i = 0; i < trial.choices.length; i++) {
      var str = buttons[i].replace(/%choice%/g, trial.choices[i]);
      html += '<div class="jspsych-dialog-button" style="display: inline-block; margin:'+trial.margin_vertical+' '+trial.margin_horizontal+'" id="jspsych-dialog-button-' + i +'" data-choice="'+i+'">'+str+'</div>';
    }
    html += '</div>';
    ...
    for (var i = 0; i < trial.choices.length; i++) {
      display_element.querySelector('#jspsych-dialog-button-' + i).addEventListener('click', function(e){
    ...
    if (trial.stimulus_duration !== null) {
      jsPsych.pluginAPI.setTimeout(function() {
        display_element.querySelector('#jspsych-dialog-stimulus').style.visibility = 'hidden';
